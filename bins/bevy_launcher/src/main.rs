...
use bevy::scene2::{bsn, on, LoadScene, Scene, ScenePatchInstance};
use bevy::tasks::Task;
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    let bsn_handle = asset_server.load_scene("scene://bsn_ui.bsn", bsn_ui());
    // ui camera
    commands.spawn(Camera2d);
    commands.spawn(ScenePatchInstance(bsn_handle));
}

fn bsn_ui() -> impl Scene {
    bsn!(
        Node {
        ...
        on(spawn_folder_dialog::<OpenProjectFileDialog>)
